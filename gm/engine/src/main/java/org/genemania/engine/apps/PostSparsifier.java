...
public class PostSparsifier extends AbstractEngineApp {
    ...
    /*
     * compute a structural overlap matrix for a selection of networks
     * for the given organism.
     */
    void processOrganism(Organism organism) throws Exception {
        total = 0;

        logger.info("computing overlap");
        ArrayList<Long> allNetworks = new ArrayList<Long>();
        allNetworks.addAll(getNetworks("*", organism));    	    	
        SymMatrix overlap = computeOverlap(organism, allNetworks);    	

        logger.info("sparsifying");
        ArrayList<Long> networksForSparsification = new ArrayList<Long>();
     ...
     */
    public SymMatrix computeOverlap(Organism organism, ArrayList<Long> networksList) throws Exception {
        SymMatrix overlap = null;      

        for (long iNetworkId: networksList) {

            Network iNetwork = getCache().getNetwork(Data.CORE, organism.getId(), iNetworkId);
            SymMatrix iData = iNetwork.getData();
    ...
    public void sparsifyAll(Organism organism, ArrayList<Long> networksList, SymMatrix overlap) throws Exception {

        for (long networkId: networksList) {

            Network network = getCache().getNetwork(Data.CORE, organism.getId(), networkId);
            SymMatrix networkData = network.getData();
            logger.info("sparsifying network " + networkId);
            SymMatrix sparsifiedNetworkData = sparsify(networkData, overlap);
    ...
    /*
     * apply sparsification based on threshold param and accumulated overlap counts
     */
    public SymMatrix sparsify(SymMatrix network, SymMatrix overlap) {
        ...
        logger.info(String.format("kept %d, removed %d, reduction %5f%%", keepers, removed, removed*100d/(keepers+removed)));                 
    ...
    public void process() throws Exception {
        ...
        if (orgId != null) {
            Organism organism = getDataConnector().getOrganismMediator().getOrganism(orgId);
     ...
     */
    private Collection<Long> getNetworks(String groupCode, Organism organism) throws ApplicationException, DataStoreException {
        ...
        for (InteractionNetworkGroup group: groups) {
            ...
            if (groupCode.equals("*") || group.getCode().equals(groupCode)) {
                ...
                for (InteractionNetwork n: networks) {
                    ...
                    logger.debug(String.format("using network %d containing %d interactions from group %s: %s",
        ...
        logger.info(String.format("total %d networks selected", ids.size()));
        return ids;
    }
