 ...
 *
 */
public class FlexDoubleMatrix extends AbstractMatrix {
    int rows;
    int cols;
    ...
    public FlexDoubleMatrix(int rows, int cols) {
        this.rows = rows;
        this.cols = cols;
    ...
    private void alloc() {
        data = new FlexDoubleArray[this.rows];
        for (int i=0; i<this.rows; i++) {
            data[i] = new FlexDoubleArray(cols);
        };
    }
    ...
    public double elementMultiplySum(Matrix m) {
        if (m instanceof FlexDoubleMatrix) {
            return elementMultiplySum((FlexDoubleMatrix) m);
        }
        else {
    ...
    public double elementMultiplySum(FlexDoubleMatrix m) {
