...
public class PrecombinedDumper implements Summarizer {
    ...
    void dump(String method, String networks, ReporterFactory reporterFactory) throws Exception {
        ...
        Map<Long, Double> weights = cnw.getWeightMap();
    ...
    void reportWeights(Map<Long, Double> weights, ReporterFactory reporterFactory, String reportName) throws Exception {
        ...
        try {
            ...
            for (Long networkId: weights.keySet()) {
                double weight = weights.get(networkId);
                InteractionNetwork network = dataConnector.getNetworkMediator().getNetwork(networkId);

                String group = getGroupForNetworkId(networkId).getName();
    ...
    String getDefaultNetworkKey(Organism organism, String networkSelection, String methodName) throws Exception {
        ...
        INetworkWeightCalculator calculator = NetworkWeightCalculatorFactory.getCalculator(Data.CORE, dataConnector.getCache(), 
                networkIds, (int) organism.getId(), null, method, NullProgressReporter.instance());
