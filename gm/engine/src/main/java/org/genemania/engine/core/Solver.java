...
public class Solver {
    ...
    public static Map<Integer, Double> solve(Matrix KtK, Vector KtT, Map<Integer, Integer> IndexToNetworkIdMap, ProgressReporter progress) throws ApplicationException {
        ...
        while (!done) {
            ...
            if (posWeights.length == 0) {
                throw new WeightingFailedException("All Netwoks Eliminated");
            }
        ...
        Map<Integer, Double> NetworkIdToWeightMap = new HashMap<Integer, Double>();
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            Integer networkId = IndexToNetworkIdMap.get(indices[i] - 1);
            if (networkId == null) {
        ...
        if (Config.instance().isNetworkWeightNormalizationEnabled()) {
            ...
            normalizeWeights(NetworkIdToWeightMap);
        }
    ...
    public static void normalizeWeights(Map<Integer, Double> networkidToWeightMap) {

        double sum = 0;

        for (Integer id: networkidToWeightMap.keySet()) {
