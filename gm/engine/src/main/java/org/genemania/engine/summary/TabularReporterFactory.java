...
public class TabularReporterFactory implements ReporterFactory {
	String location;
	
	public TabularReporterFactory(String reportLocation) {
		this.location = reportLocation;
	}
	
	public Reporter getReporter(String reportName) throws ApplicationException {
		try {
			return new TabularReporter(location + File.separator + reportName + ".txt");
		}
		catch (IOException e) {
