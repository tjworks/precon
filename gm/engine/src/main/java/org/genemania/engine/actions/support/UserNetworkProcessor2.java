 ...
 *
 */
public class UserNetworkProcessor2 {
    private static Logger logger = Logger.getLogger(UserNetworkProcessor2.class);
    private DataCache cache;
    private String tempDirname;

    public UserNetworkProcessor2(DataCache cache, String tempDirname) {
...
//        CacheNetworkToMappingBuilder mappingBuilder = new CacheNetworkToMappingBuilder(organismId, cache);
//        final Mapping<String, Integer> myMapping = mappingBuilder.getMapping();
//
//        FileNetworkMatrixProvider provider = new FileNetworkMatrixProvider(organismId, getUserTempDirname(namespace), myMapping);
//        Matrix m = provider.getNetworkMatrix(networkId, NullProgressReporter.instance());
//        return m;
    }
...
//        cache.putUserNetwork(request.getNamespace(), request.getOrganismId(), request.getNetworkId(), network);
    }
    ...
    void precompute(UploadNetworkEngineRequestDto request, SymMatrix network, ProgressReporter progress) throws ApplicationException {
        UserDataPrecomputer2 precomputer = new UserDataPrecomputer2(request.getNamespace(), (int) request.getOrganismId(), cache, request.getProgressReporter());
