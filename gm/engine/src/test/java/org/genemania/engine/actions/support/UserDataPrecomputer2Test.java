...
import org.genemania.engine.actions.support.UserDataPrecomputer2;
...
public class UserDataPrecomputer2Test {
    ...
    public void setUp() throws ApplicationException {
        ...
        networkIds = randomCacheBuilder.addOrganism(1, 10, 3, .5, 5, .5);
    ...
    public void testCheckNetworkInvalidId() throws Exception {
        ...
        int organismId = 1;
        int id = 1;
        Map<Integer, Integer> userColumnMap = new HashMap();
...
//        INetworkSymMatrixProvider provider = new CacheNetworkSymMatrixProvider(1, randomCacheBuilder.getCache());
        UserDataPrecomputer2 instance = new UserDataPrecomputer2(namespace, organismId, randomCacheBuilder.getCache(), NullProgressReporter.instance());
    ...
    public void testCheckDuplicateId() throws Exception {
        ...
        String namespace = "user1";
        int organismId = 1;
        long id1 = networkIds[0];
        UserDataPrecomputer2 instance = null;

        try {
            instance = new UserDataPrecomputer2(namespace, organismId, randomCacheBuilder.getCache(), NullProgressReporter.instance());

            // pull out one of the existing networks, and add back in as a user network
            System.out.println("copying network " + id1);
            SymMatrix userNetwork1 = randomCacheBuilder.getCache().getNetwork(Data.CORE, organismId, id1).getData();
            ...
            randomCacheBuilder.getCache().putNetwork(userNetworkObj1);
    ...
    public void testCheckAllOK() throws Exception {
        ...
        userColumnMap.put(-2, 1);
        userColumnMap.put(-5, 2);
//        INetworkMatrixProvider provider = new CacheNetworkMatrixProvider(1, randomCacheBuilder.getCache());
        UserDataPrecomputer2 instance = new UserDataPrecomputer2(namespace, organismId, randomCacheBuilder.getCache(), NullProgressReporter.instance());
        instance.load();
        instance.checkNetwork(id);
    }
...
//        INetworkMatrixProvider provider = new CacheNetworkMatrixProvider(1, randomCacheBuilder.getCache());
    ...
    public void testAddNetwork() throws Exception {
        ...
        String namespace = "user1";
        int organismId = 1;
//        INetworkMatrixProvider provider = new CacheNetworkMatrixProvider(namespace, 1, randomCacheBuilder.getCache());
        UserDataPrecomputer2 instance = new UserDataPrecomputer2(namespace, organismId, randomCacheBuilder.getCache(), NullProgressReporter.instance());
        
        // pull out one of the existing networks, and add back in as a user network
        long id = networkIds[0];
        System.out.println("copying network " + id);
        SymMatrix userNetwork = randomCacheBuilder.getCache().getNetwork(Data.CORE, organismId, id).getData();
        int userNetworkId = -1;

        Network userNetworkObj = new Network(namespace, organismId, userNetworkId);
        userNetworkObj.setData(userNetwork);
        randomCacheBuilder.getCache().putNetwork(userNetworkObj);

        instance.addNetwork(userNetworkId, userNetwork);

        // check the column id map
//        Map<Integer, Integer> coreMap = randomCacheBuilder.getCache().getColumnId(organismId);
//        Map<Integer, Integer> userMap = randomCacheBuilder.getCache().getUserColumnId(namespace, organismId);
...
//        assertEquals(userMap.size()-1, (long) userMap.get(userNetworkId));

        // all other elements should be the same
//        for (int netId: coreMap.keySet()) {
...
//            int coreIndex = coreMap.get(netId);
            int userIndex = userNetworkIds.getIndexForId(netId);
            assertEquals(coreIndex, userIndex);            
        }
        
        // check the user's precomputed KtK
        Matrix userKtK_BASIC = randomCacheBuilder.getCache().getKtK(namespace, organismId, Constants.DataFileNames.KtK_BASIC.getCode()).getData();
        assertNotNull(userKtK_BASIC);
        //System.out.println("user ktk basic: " + userKtK_BASIC);
        // compare with original KtK
        Matrix KtK_BASIC = randomCacheBuilder.getCache().getKtK(Data.CORE, organismId, Constants.DataFileNames.KtK_BASIC.getCode()).getData();
...
//        INetworkMatrixProvider provider = new CacheNetworkMatrixProvider(namespace, 1, randomCacheBuilder.getCache());
        ...
        // and another one
        long id2 = networkIds[1];
        System.out.println("copying network " + id2);
        SymMatrix userNetwork2 = randomCacheBuilder.getCache().getNetwork(Data.CORE, organismId, id2).getData();
        ...
        randomCacheBuilder.getCache().putNetwork(userNetworkObj2);
        ...
        //System.out.println("user: " + userKtK_BASIC);
