...
public class UserNetworkProcessorSimultCombing2Test {
    ...
    @Test
    public void testAutoRelCombiningWithUserNetwork() throws Exception {
        RelatedGenesEngineResponseDto relatedResponse = regressionHelper(CombiningMethod.AUTOMATIC_RELEVANCE);
        assertNotNull(relatedResponse);
        assertNotNull(relatedResponse.getNetworks());
        assertFalse(relatedResponse.getNetworks().size() == 0);
    }
...
//        INetworkSymMatrixProvider provider = new CacheNetworkSymMatrixProvider(userName, orgId, randomCacheBuilder.getCache());
        UserNetworkProcessor2 instance = new UserNetworkProcessor2(randomCacheBuilder.getCache(), randomCacheBuilder.getCacheDir());
