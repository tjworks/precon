...
import org.genemania.engine.apps.NodeDegreeComputer;
...
public class RandomDataCacheBuilder {
    ...
    public long[] addOrganism(long organismId, int numGenes, int numNetworks, double sparsity) throws ApplicationException {
        return addOrganism(organismId, numGenes, numNetworks, sparsity, 0, 0);
    }
...
//        getCache().putColumnId(organismId, map);
    ...
    private void putRandomAnnotationData(long organismId, long [] networkIds, int numGenes, int numCategories, double sparsity) throws ApplicationException, DataStoreException {
        ...
        Matrix [] allLabels = new Matrix[3];
        Map[] allIndexMaps = new Map[3];
        ...
        for (int branch = 0; branch < Constants.goBranches.length; branch++) {
            ...
            allLabels[branch] = labels;
            allIndexMaps[branch] = goIndexMap;
        }

        computeSimultCombiningConstants(organismId, networkIds, allLabels, allIndexMaps);
    }

    public void computeSimultCombiningConstants(long organismId, long[] networkIds, Matrix [] labels, Map[] goIndexMap) throws ApplicationException, DataStoreException {
        FastWeightCacheBuilder fwcb = new FastWeightCacheBuilder();
        fwcb.setCacheDir(cache.getCacheDir());
        fwcb.setCache(getCache());
        fwcb.setMethod(FastWeightCacheBuilder.METHOD_SIMPLE); // seems to be faster than METHOD_FAST
        ...
        try {
            fwcb.buildFastWeightDataForOrganism(organism, labels, goIndexMap, NullProgressReporter.instance());
        }
        catch (Exception e) {
    ...
    /* contains node ids, numbered from organismId*NUBMERING_CONST
     *
     */
    ...
    /*
     * return symmetrix sparse matrix with zero diagonal and given
