...
public class AbstractPearsonTest extends TestCase{

	public void testGetZeroRank(){
		// v = {1, 2, 3, 4, 5}
		Vector v = new DenseVector(new double[]{1, 2, 3, 4, 5});
		assertEquals( AbstractPearson.getZeroRank(v), 0.0 );
	
		// v = { , , 1, 1, }		
		v = new SparseVector(5, new int[]{2, 3}, new double[]{1,1});
		assertEquals( AbstractPearson.getZeroRank(v), 2.0);
		
		v = new SparseVector(5, new int[]{2}, new double[]{1});
		assertEquals( AbstractPearson.getZeroRank(v), 2.5);
	}
}
