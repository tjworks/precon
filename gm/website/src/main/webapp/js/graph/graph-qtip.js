...
CytowebUtil._makeTooltip = function(evt){
    ...
    var prev_qtip;
    if( evt.group == "nodes" ){
        var prev_qtip = $(".qtip[nodeid=" + sanitise(evt.target.data.id) + "]:visible");
    } else {  
        var prev_qtip = $(".qtip[edgeid=" + sanitise(evt.target.data.id) + "]:visible");
    }
    ...
    $("body").children(".qtip:last").each(function(){
        ...
        if( evt.group == "nodes" ){
            ...
            qtip.attr("nodeid", sanitise(evt.target.data.id));
        } else {
            ...
            qtip.attr("edgeid", sanitise(evt.target.data.id));
        }
