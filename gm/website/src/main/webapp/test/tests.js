testSuite(function(){
	...
	testCase("Links", function(){
		function link_is_local(url){
			return !url.match(/http\:\/\//);
		}
		var all_links = [];
		var link_to_sources = {};
		function get_links(html, page){
			var links = [];
			
			$(html).find("a").each(function(){
				var url = $(this).attr("href");
				if( link_is_local(url) && !url.match(/\#/) ){
					if( link_to_sources[url] == undefined ){
						link_to_sources[url] = [];
					}
					if( jQuery.inArray(page, link_to_sources[url]) < 0 ){
						link_to_sources[url].push(page);
					}
					
					if( jQuery.inArray(url, all_links) < 0 ){
						links.push(url);
						all_links.push(url);
					}
				}
		...
		var visited_links = [];
		var remaining_links = get_links( gm$("html").html() , "/" );
		
		while( remaining_links.length > 0 ){
			var current_link = remaining_links.shift();
			
			$.ajax({
				async: false,
				url: current_link,
				success: function(html){
					ok(true, "URL " + current_link + " is OK");	
					if( link_is_local(current_link) ){
						var new_links = get_links( html, current_link );
						for(var i = 0; i < new_links.length; i++){
							remaining_links.push( new_links[i] );
						}
					}
				},
				error: function(){
					ok(false, "URL " + current_link + " is dead from source URL set [" + link_to_sources[current_link] + "]");
				}
			});
			visited_links.push(current_link);
		}
		nextTestCase();
	});
					...
					if (gm$("label:contains('" + network_name + "'):last").parent().children(".uploadError").is(":visible")){
						...
						runSpecificDefaultQuery(HUMAN_ID, function(){
							if (gm$(".checktree_top_level:contains('Uploaded')").is(":visible")){
								...
								ok(pass, "The uploaded networks should be shown in the color " + uploaded_color + " in the graph");
								nextTestCase();
							}
							else{
