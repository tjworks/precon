...
public class JmsEngineConnector implements EngineConnector {
	...
	private RelatedGenesWebResponseDto load(
		...
		Map<Long, Node> nodeLookup = new HashMap<Long, Node>();
		...
		for (InteractionNetwork hollowNetwork : hollowResponseDto.getNetworks()) {
			...
			Collection<Interaction> interactions = hollowNetwork
					.getInteractions();

			for (Interaction hollowInteraction : interactions) {
				Node hollowFromNode = hollowInteraction.getFromNode();
				long fromNodeId = hollowFromNode.getId();

				// cache node, to avoid loading it from the DB again
				Node fromNode = nodeLookup.get(fromNodeId);

				if (fromNode == null) {
					fromNode = nodeDao.findNode(hollowResponseDto.getOrganismId(), fromNodeId);
					if (fromNode != null)
						nodeLookup.put(fromNodeId, fromNode);
				}

				if (fromNode != null) {
					if (fromNode.getGeneData() != null) {
						hollowFromNode.setGeneData(fromNode.getGeneData());
						hollowFromNode.setGenes(fromNode.getGenes());
						hollowFromNode.setName(fromNode.getName());
						hollowInteraction.setFromNode(hollowFromNode);
					}
				} else {
					LOG.warn("Cannot load \"from\" node with id=" + fromNodeId);
				}

				Node hollowToNode = hollowInteraction.getToNode();
				long toNodeId = hollowToNode.getId();

				Node toNode = nodeLookup.get(toNodeId);

				if (toNode == null) {
					toNode = nodeDao.findNode(hollowResponseDto.getOrganismId(), toNodeId);
					if (toNode != null)
						nodeLookup.put(toNodeId, toNode);
				}

				if (toNode != null) {
					if (toNode.getGeneData() != null) {
						hollowToNode.setGeneData(toNode.getGeneData());
						hollowToNode.setGenes(toNode.getGenes());
						hollowToNode.setName(toNode.getName());
						hollowInteraction.setToNode(hollowToNode);
					}
				} else {
					LOG.warn("Cannot load \"to\" node with id=" + toNodeId);
				}
			}
		}
