(function(){
var format = function(digit){ return digit>9? digit: '0'+digit }
var timestamp = function(){ var d = new Date(); return format(d.getHours())+':'+ format(d.getMinutes())+":"+format(d.getSeconds())+"."+ d.getMilliseconds() }	
window.Log = function(level,logger,prefix){
var _currentLevel=Log.WARN;var _logger=Log.writeLogger;var _prefix=false;this.setPrefix=function(pre){if(pre!='undefined'){_prefix=pre;}
else{_prefix=false;}}
...
Log.consoleLogger=function(msg,level,obj){if(window.console){window.console.log(timestamp() + " "+ level+" - "+msg);}else{Log.popupLogger(msg,level,obj);}}
